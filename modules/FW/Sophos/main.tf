...
# Sophos FW
resource "azurerm_virtual_machine" "SophosFW" {
  name                          = "SophosXG"
  resource_group_name           = var.resource_group_name
  location                      = var.RGlocation
...
# Configure Auto-Shutdown for the Sophos VM for each night at 10pm CST.
resource "azurerm_dev_test_global_vm_shutdown_schedule" "SophosShutdown" {
  virtual_machine_id = azurerm_virtual_machine.SophosFW.id
  location           = var.RGlocation
  enabled            = true
  ...
  depends_on = [
    azurerm_virtual_machine.SophosFW
  ]
}
