...
# Watchguard FW
resource "azurerm_virtual_machine" "WatchguardFW" {
  name                          = "WatchguardFW"
  resource_group_name           = var.resource_group_name
  location                      = var.RGlocation
...
# Configure Auto-Shutdown for the Watchguard VM for each night at 10pm CST.
resource "azurerm_dev_test_global_vm_shutdown_schedule" "WatchguardShutdown" {
  virtual_machine_id = azurerm_virtual_machine.WatchguardFW.id
  location           = var.RGlocation
  enabled            = true
  ...
  depends_on = [
    azurerm_virtual_machine.WatchguardFW
  ]
}
